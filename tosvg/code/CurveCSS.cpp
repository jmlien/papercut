...
inline double myArcLength(vector<Point2d>& pl)
{
	auto pre = pl.begin();
	double len = 0;
...
void ResampleCurve(const vector<double>& curvex, const vector<double>& curvey,
	...
	double pl_length = myArcLength(pl); // arcLength(Mat(pl), false);
...
//		cout << curr << " and " << curr+1 << "\t\t" << last_dist << " ("<<dist<<")"<<endl;
		if (dist >= resample_size) {
			...
			//
			// BUG, cp1 and cp can have the same values....WHY???
			// 
			Point2d dirv = cp1-cp; 


			dirv = dirv * (1.0 / norm(dirv));
...
//				cout << "point " << i << " between " << curr << " and " << curr+1 << " remaining " << dist << endl;
			...
			if (resamplepl[i-1].x != resamplepl[i-1].x)
			{
				cout << "cp=" << cp << " cp1="<<cp1<<" dirv=" << dirv << " _d=" << _d << endl;
			}
		}
	...
	for (int i = 0; i < N;i++)
		if (resamplepl[i].x != resamplepl[i].x)
			cout << "resamplepl[" << i << "] has problem" << endl;
					 ...
					 ) 
{
	...
	for (int i=0; i<curvex.size(); i++) {
		// Mokhtarian 02' eqn (4)
		kappa[i] = (X[i] * YY[i] - XX[i] * Y[i]) / std::pow(X[i] * X[i] + Y[i] * Y[i], 1.5);
	}
}
...
//					circle(contourimg, contours[0][crossings[c]], 5, Scalar(0,0,255), CV_FILLED);
					...
					if (c < crossings.size()-1) {
						if (abs(crossings[c] - crossings[c+1]) < 5.0) {
							//this is a maxima
